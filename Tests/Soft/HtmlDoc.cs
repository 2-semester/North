...
namespace North.Tests.Soft {
    ...
    public static class HtmlDoc {
        ...
        public static HttpRequestMessage ComposeRequestMessage<TData>(
            ...
            if (!(item is null)) foreach (var p in typeof(TData).GetProperties()) {
                var v = HtmlHelper.Value(p.GetValue(item), true);
                var id = $"Item_{p.Name}";
                var element = form.Elements.GetElementById(id);
                if (element is IHtmlSelectElement)
                    ((IHtmlSelectElement)element).Value = v;
                else
                    ((IHtmlInputElement)element).Value = v;
            }
        ...
        public static HttpRequestMessage ComposeRequestMessage(
            IHtmlFormElement form,
            IHtmlElement submitButton){ 
